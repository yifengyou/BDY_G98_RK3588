...
# apply patch
if ls "${WORKDIR}/ophub_6.18.y/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/ophub_6.18.y/*.patch
fi

if [ -d ${WORKDIR}/ophub_6.18.y ]; then
  ls -alh ${WORKDIR}/ophub_6.18.y/
  cp -a ${WORKDIR}/ophub_6.18.y/* .
  ls -alh
fi
