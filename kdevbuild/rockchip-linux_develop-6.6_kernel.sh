...
# 功能：编译 U-Boot、Linux develop-6.6 内核、模块、头文件及 kernel-devel 包
# ========================================================================
...
LOG_FILE="${WORKDIR}/build-develop-6.6.log"
...
# 【注意】目录名含 .git 后缀可能导致部分工具误判，建议改为 develop-6.6
KERNEL_SRC_DIR="kernel_develop-6.6"
git clone https://github.com/rockchip-linux/kernel.git -b develop-6.6 "${KERNEL_SRC_DIR}"
cd "${KERNEL_SRC_DIR}"
ls -alh

# 如果存在补丁目录，则通过 git am 应用所有补丁
PATCH_DIR="${WORKDIR}/rockchip-linux_develop-6.6"
...
cp -a arch/arm64/boot/Image "${WORKDIR}/release/Image-develop-6.6-kdev"
...
# --- 内核配置文件 ---
cp .config "${WORKDIR}/release/config-develop-6.6-kdev"
ls -alh "${WORKDIR}/release/config-develop-6.6-kdev"
md5sum "${WORKDIR}/release/config-develop-6.6-kdev"

# --- System.map ---
cp System.map "${WORKDIR}/release/System.map-develop-6.6-kdev"
ls -alh "${WORKDIR}/release/System.map-develop-6.6-kdev"
md5sum "${WORKDIR}/release/System.map-develop-6.6-kdev"
...
if [ -d kos/lib/modules ]; then
  ...
  if [ "$dir_size" -gt 512 ]; then
    ...
    mkdir -p "${WORKDIR}/release"
    tar -zcvf "${WORKDIR}/release/kos-develop-6.6.tar.gz" kos
  fi
fi
...
if [ -f vmlinux ]; then
  ...
  if [ ${#DEBUGINFO_FILES[@]} -gt 0 ]; then
    tar -zcvf "${WORKDIR}/release/kernel-debuginfo-develop-6.6.tar.gz" "${DEBUGINFO_FILES[@]}"
...
if [ -d kernel-headers ]; then
  mkdir -p "${WORKDIR}/release"
  tar -zcvf "${WORKDIR}/release/kernel-headers-develop-6.6.tar.gz" kernel-headers
fi
...
# 打包 kernel-devel
cd "${WORKDIR}"
tar -czf "${WORKDIR}/release/kernel-devel-develop-6.6.tar.gz" kernel-devel
