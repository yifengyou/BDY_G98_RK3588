...
# apply patch
if ls "${WORKDIR}/ophub_6.12.y/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/ophub_6.12.y/*.patch
fi

if [ -d ${WORKDIR}/ophub_6.12.y ]; then
  ls -alh ${WORKDIR}/ophub_6.12.y/
  cp -a ${WORKDIR}/ophub_6.12.y/* .
  ls -alh
fi
...
ls -alh ${WORKDIR}/release/config-6.12.y-kdev
md5sum ${WORKDIR}/release/config-6.12.y-kdev

# release system map
cp System.map ${WORKDIR}/release/System.map-6.12.y-kdev
ls -alh ${WORKDIR}/release/System.map-6.12.y-kdev
md5sum ${WORKDIR}/release/System.map-6.12.y-kdev
