...
# apply patch
if ls "${WORKDIR}/rockchip-linux_develop-6.1/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/rockchip-linux_develop-6.1/*.patch
fi

if [ -d ${WORKDIR}/rockchip-linux_develop-6.1 ]; then
  ls -alh ${WORKDIR}/rockchip-linux_develop-6.1/
  cp -a ${WORKDIR}/rockchip-linux_develop-6.1/* .
  ls -alh
fi
