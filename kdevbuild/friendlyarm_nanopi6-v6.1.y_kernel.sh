...
# apply patch
if ls "${WORKDIR}/friendlyarm_nanopi6-v6.1.y/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/friendlyarm_nanopi6-v6.1.y/*.patch
fi

if [ -d ${WORKDIR}/friendlyarm_nanopi6-v6.1.y ]; then
  ls -alh ${WORKDIR}/friendlyarm_nanopi6-v6.1.y/
  cp -a ${WORKDIR}/friendlyarm_nanopi6-v6.1.y/* .
  ls -alh
fi
