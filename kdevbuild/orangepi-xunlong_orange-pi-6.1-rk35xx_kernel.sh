...
#==========================================================================#
cd ${WORKDIR}
git clone -b orange-pi-6.1-rk35xx https://github.com/orangepi-xunlong/linux-orangepi orangepi-xunlong_orange-pi-6.1-rk35xx.git
cd orangepi-xunlong_orange-pi-6.1-rk35xx.git
ls -alh

# apply patch
if ls "${WORKDIR}/orangepi-xunlong_orange-pi-6.1-rk35xx/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/orangepi-xunlong_orange-pi-6.1-rk35xx/*.patch
fi

if [ -d ${WORKDIR}/orangepi-xunlong_orange-pi-6.1-rk35xx ]; then
  ls -alh ${WORKDIR}/orangepi-xunlong_orange-pi-6.1-rk35xx/
  cp -a ${WORKDIR}/orangepi-xunlong_orange-pi-6.1-rk35xx/* .
  ls -alh
fi
