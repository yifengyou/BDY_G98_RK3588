...
# apply patch
if ls "${WORKDIR}/lubancat_lbc-develop-6.1/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/lubancat_lbc-develop-6.1/*.patch
fi

if [ -d ${WORKDIR}/lubancat_lbc-develop-6.1 ]; then
  ls -alh ${WORKDIR}/lubancat_lbc-develop-6.1/
  cp -a ${WORKDIR}/lubancat_lbc-develop-6.1/* .
  ls -alh
fi
