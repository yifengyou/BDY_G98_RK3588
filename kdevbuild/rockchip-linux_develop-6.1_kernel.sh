...
# 功能：编译 U-Boot、Linux develop-6.1 内核、模块、头文件及 kernel-devel 包
# ========================================================================
...
LOG_FILE="${WORKDIR}/build-develop-6.1.log"
...
PATCH_DIR="${WORKDIR}/rockchip-linux_develop-6.1"
...
cp -a arch/arm64/boot/Image "${WORKDIR}/release/Image-develop-6.1-kdev"
...
# --- 内核配置文件 ---
cp .config "${WORKDIR}/release/config-develop-6.1-kdev"
ls -alh "${WORKDIR}/release/config-develop-6.1-kdev"
md5sum "${WORKDIR}/release/config-develop-6.1-kdev"

# --- System.map ---
cp System.map "${WORKDIR}/release/System.map-develop-6.1-kdev"
ls -alh "${WORKDIR}/release/System.map-develop-6.1-kdev"
md5sum "${WORKDIR}/release/System.map-develop-6.1-kdev"
...
if [ -d kos/lib/modules ]; then
  ...
  if [ "$dir_size" -gt 512 ]; then
    ...
    mkdir -p "${WORKDIR}/release"
    tar -zcvf "${WORKDIR}/release/kos-develop-6.1.tar.gz" kos
  fi
fi
...
if [ -f vmlinux ]; then
  ...
  if [ ${#DEBUGINFO_FILES[@]} -gt 0 ]; then
    tar -zcvf "${WORKDIR}/release/kernel-debuginfo-develop-6.1.tar.gz" "${DEBUGINFO_FILES[@]}"
...
if [ -d kernel-headers ]; then
  mkdir -p "${WORKDIR}/release"
  tar -zcvf "${WORKDIR}/release/kernel-headers-develop-6.1.tar.gz" kernel-headers
fi
...
# 打包 kernel-devel
cd "${WORKDIR}"
tar -czf "${WORKDIR}/release/kernel-devel-develop-6.1.tar.gz" kernel-devel
