...
# 功能：编译 U-Boot、Linux linux-6.1.y-rockchip 内核、模块、头文件及 kernel-devel 包
# ========================================================================
...
LOG_FILE="${WORKDIR}/build-linux-6.1.y-rockchip.log"
...
# 【注意】目录名含 .git 后缀可能导致部分工具误判，建议改为 linux-linux-6.1.y-rockchip
KERNEL_SRC_DIR="linux-linux-6.1.y-rockchip"
git clone https://github.com/ophub/linux-6.1.y-rockchip.git -b main "${KERNEL_SRC_DIR}"
cd "${KERNEL_SRC_DIR}"
ls -alh

# 如果存在补丁目录，则通过 git am 应用所有补丁
PATCH_DIR="${WORKDIR}/ophub_linux-6.1.y-rockchip"
...
cp -a arch/arm64/boot/Image "${WORKDIR}/release/Image-linux-6.1.y-rockchip-kdev"
...
# --- 内核配置文件 ---
cp .config "${WORKDIR}/release/config-linux-6.1.y-rockchip-kdev"
ls -alh "${WORKDIR}/release/config-linux-6.1.y-rockchip-kdev"
md5sum "${WORKDIR}/release/config-linux-6.1.y-rockchip-kdev"

# --- System.map ---
cp System.map "${WORKDIR}/release/System.map-linux-6.1.y-rockchip-kdev"
ls -alh "${WORKDIR}/release/System.map-linux-6.1.y-rockchip-kdev"
md5sum "${WORKDIR}/release/System.map-linux-6.1.y-rockchip-kdev"
...
if [ -d kos/lib/modules ]; then
  ...
  if [ "$dir_size" -gt 512 ]; then
    ...
    mkdir -p "${WORKDIR}/release"
    tar -zcvf "${WORKDIR}/release/kos-linux-6.1.y-rockchip.tar.gz" kos
  fi
fi
...
if [ -f vmlinux ]; then
  ...
  if [ ${#DEBUGINFO_FILES[@]} -gt 0 ]; then
    tar -zcvf "${WORKDIR}/release/kernel-debuginfo-linux-6.1.y-rockchip.tar.gz" "${DEBUGINFO_FILES[@]}"
...
if [ -d kernel-headers ]; then
  mkdir -p "${WORKDIR}/release"
  tar -zcvf "${WORKDIR}/release/kernel-headers-linux-6.1.y-rockchip.tar.gz" kernel-headers
fi
...
# 打包 kernel-devel
cd "${WORKDIR}"
tar -czf "${WORKDIR}/release/kernel-devel-linux-6.1.y-rockchip.tar.gz" kernel-devel
